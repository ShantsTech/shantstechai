...
export function ShantsTechLinkPopover() {
  return (
...
function LinkDropdown() {
  ...
  const { instance, instances, status } = useShantsTechLink();
    ...
    <>
      <Stack py="sm" px="lg" spacing={4}>
        ...
        <Text my="xs">
          ...
          instance in realtime from ShantsTech
        </Text>
      </Stack>
      <Divider />
      <Stack>
...
function GetReconnected() {
  const { instance, createInstance } = useShantsTechLink();
...
function ActivityList() {
  const ids = useShantsTechLinkStore((state) => state.ids);
  const { classes } = useStyles();
  return ids.length > 0 ? (
    <ScrollArea.Autosize maxHeight={410}>
      {ids.map((id) => (
...
function LinkActivity({ id, ...props }: { id: string } & GroupProps) {
  const activity = useShantsTechLinkStore(useCallback((state) => state.activities[id], [id]));
  const { runCommand } = useShantsTechLink();
