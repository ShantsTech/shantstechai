...
import { CommandResourcesAdd } from '~/components/ShantsTechLink/shared-types';
...
export function ShantsTechLinkResourceManager({
  modelId,
  modelName,
  modelType,
  modelVersionId,
  ...
  hashes: string[];
  children: (props: ShantsTechLinkResourceManagerChildrenFunction) => JSX.Element;
  fallback?: JSX.Element;
}) {
  const { connected, resources, runCommand } = useShantsTechLink();
  const resource = resources.find(({ hash }) => hashes.some((x) => x === hash));
  const activities = useShantsTechLinkStore(
    useCallback(
      (state) =>
...
export type ShantsTechLinkResourceManagerProps = {
  modelId: number;
  modelName: string;
  modelType: ModelType;
  modelVersionId?: number;
...
type ShantsTechLinkResourceManagerChildrenFunction = {
  addResource: () => void;
  removeResource: () => void;
  cancelDownload: () => void;
